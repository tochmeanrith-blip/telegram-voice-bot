...
2. Time: "៩ ព្រឹក"→"09:00", "២ រសៀល"→"14:00"
3. Category ត្រូវជ្រើសរើសសមរម្យ
4. Event ត្រូវកាត់ date/time words ចេញ
...
* {{ box-sizing: border-box; margin: 0; padding: 0; letter-spacing: normal; word-spacing: normal; }}
html, body {{ font-family: 'Noto Sans Khmer', sans-serif; color: #202124; font-size: 10pt; }}
.day-num, .cell-num, .weekday-en, .wd-en, .event-id, .event-time, .header-count-num {{
    font-family: 'Arial', sans-serif !important; letter-spacing: 0 !important;
}}
.header {{ background-color: #4285F4; color: white; padding: 20px 25px; border-radius: 8px 8px 0 0; }}
.header-flex {{ display: table; width: 100%; }}
.header-left {{ display: table-cell; vertical-align: middle; width: 70%; }}
.header-right {{ display: table-cell; vertical-align: middle; text-align: right; font-size: 12pt; font-weight: 500; }}
.header-title {{ font-size: 24pt; font-weight: 700; margin-bottom: 6px; line-height: 1.2; }}
.header-subtitle {{ font-size: 13pt; font-weight: 400; opacity: 0.95; }}
.grid {{ display: table; width: 100%; border-collapse: separate; border-spacing: 4px; margin-top: 4px; table-layout: fixed; }}
.grid-row {{ display: table-row; }}
.day-col {{ display: table-cell; background-color: white; border: 1px solid #DADCE0; border-radius: 6px; vertical-align: top; padding: 10px 8px; width: 14.28%; min-height: 460px; }}
.day-col.weekend {{ background-color: #FFF3E0; }}
.day-col.today {{ background-color: #E3F2FD; }}
.day-header {{ text-align: center; padding-bottom: 10px; border-bottom: 1px solid #DADCE0; margin-bottom: 10px; }}
.weekday-en {{ font-size: 10pt; font-weight: 700; color: #5F6368; }}
.weekday-en.weekend-text {{ color: #EA4335; }}
.weekday-en.today-text {{ color: #4285F4; }}
.day-num {{ font-size: 24pt; font-weight: 700; color: #202124; margin: 6px 0; line-height: 1; }}
.day-num.today-circle {{ display: inline-block; background-color: #4285F4; color: white; width: 44px; height: 44px; line-height: 44px; border-radius: 50%; font-size: 18pt; text-align: center; padding: 0; margin: 4px auto; }}
.weekday-kh {{ font-size: 9pt; color: #5F6368; margin-top: 4px; }}
.events {{ padding-top: 4px; }}
.event {{ color: white; border-radius: 5px; padding: 6px 8px; font-size: 8pt; line-height: 1.4; margin-bottom: 5px; page-break-inside: avoid; }}
.event-header {{ width: 100%; display: table; table-layout: fixed; margin-bottom: 5px; padding-bottom: 3px; font-size: 7.5pt; font-weight: 700; border-bottom: 1px solid rgba(255,255,255,0.25); }}
.event-id {{ display: table-cell; text-align: left; vertical-align: middle; width: 50%; }}
.event-time {{ display: table-cell; text-align: right; vertical-align: middle; width: 50%; }}
.event-text {{ font-size: 9pt; line-height: 1.4; word-wrap: break-word; overflow-wrap: break-word; }}
.footer {{ margin-top: 10px; display: table; width: 100%; font-size: 8pt; color: #5F6368; padding: 5px; }}
.footer-left {{ display: table-cell; vertical-align: middle; }}
.footer-right {{ display: table-cell; vertical-align: middle; text-align: right; }}
.legend-item {{ display: inline-block; margin-right: 15px; }}
.legend-color {{ display: inline-block; width: 10px; height: 10px; border-radius: 2px; vertical-align: middle; margin-right: 4px; }}
</style>
</head>
<body>
<div class="header">
...
<div class="header-right">សរុប៖ <span class="header-count-num">{total}</span> ព្រឹត្តិការណ៍</div>
</div>
</div>
<div class="grid"><div class="grid-row">{columns_html}</div></div>
<div class="footer">
...
<div class="footer-right">Voice Tracker Bot • {footer_time}</div>
</div>
</body>
</html>"""
    return html
...
@page {{ size: A4 landscape; margin: 8mm; }}
* {{ box-sizing: border-box; margin: 0; padding: 0; letter-spacing: normal; word-spacing: normal; }}
html, body {{ font-family: 'Noto Sans Khmer', sans-serif; color: #202124; }}
.cell-num, .wd-en, .header-count-num {{ font-family: 'Arial', sans-serif !important; letter-spacing: 0 !important; }}
.header {{ background-color: #4285F4; color: white; padding: 20px; text-align: center; border-radius: 8px 8px 0 0; }}
.header-title {{ font-size: 26pt; font-weight: 700; line-height: 1.2; }}
.header-subtitle {{ font-size: 12pt; margin-top: 6px; }}
.weekdays {{ display: table; width: 100%; border-collapse: separate; border-spacing: 2px; margin-top: 4px; table-layout: fixed; }}
.wd-cell {{ display: table-cell; background-color: #4285F4; color: white; padding: 10px 4px; text-align: center; width: 14.28%; }}
.wd-en {{ font-size: 11pt; font-weight: 700; line-height: 1.2; }}
.wd-kh {{ font-size: 9pt; margin-top: 3px; }}
.grid-row {{ display: table; width: 100%; border-collapse: separate; border-spacing: 2px; table-layout: fixed; }}
.cell {{ display: table-cell; background-color: white; border: 1px solid #DADCE0; height: 90px; padding: 6px; vertical-align: top; width: 14.28%; }}
.cell.weekend {{ background-color: #FFF3E0; }}
.cell.today {{ background-color: #E3F2FD; }}
.cell.other-month {{ background-color: #F5F5F5; }}
.cell-num {{ font-size: 13pt; font-weight: 700; color: #202124; margin-bottom: 4px; display: inline-block; line-height: 1; }}
.cell-num.weekend-num {{ color: #EA4335; }}
.cell-num.today-num {{ background-color: #4285F4; color: white; width: 26px; height: 26px; line-height: 26px; text-align: center; border-radius: 50%; font-size: 11pt; }}
.other-month .cell-num {{ color: #BDBDBD; }}
.cell-events {{ margin-top: 2px; }}
.mini-event {{ color: white; font-size: 7.5pt; padding: 2px 5px; border-radius: 2px; margin-bottom: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; line-height: 1.3; }}
.more {{ font-size: 7pt; color: #5F6368; font-style: italic; margin-top: 2px; }}
.footer {{ margin-top: 10px; text-align: center; font-size: 8pt; color: #5F6368; }}
</style>
</head>
<body>
<div class="header">
...
<div class="weekdays">{weekdays_html}</div>
{rows_html}
<div class="footer">Voice Tracker Bot • {footer_time}</div>
</body>
</html>"""
    return html
...
def generate_weekly_summary_ai(week_start, week_end):
    try:
        ...
        for e in events:
            try:
                d = datetime.strptime(e['date'], "%Y-%m-%d").date()
                if prev_week_start <= d <= prev_week_end:
                    prev_week_count += 1
            except Exception:
                pass

        current_week_data = [
            {"date": e['date'], "time": e['time'], "event": e['event'],
...
{{
  ...
  "motivation": "សារលើកទឹកចិត្ត"
}}

ភាសាខ្មែរធម្មជាតិ។ ឆ្លើយតែ JSON។
"""
...
def generate_daily_briefing_ai():
    try:
        ...
        prompt = f"""អ្នកគឺជាជំនួយការ AI ជាភាសាខ្មែរ។ សូមផ្តល់ daily briefing។
...
{{
  ...
  "day_overview": "ការវាយតម្លៃថ្ងៃនេះ",
  "priority_events": ["Event សំខាន់ 1"],
  "tips": ["Tip 1", "Tip 2"],
  "tomorrow_preview": "សរុបថ្ងៃស្អែក",
...
def generate_insights_ai():
    try:
        ...
        prompt = f"""អ្នកគឺជា AI data analyst ជាភាសាខ្មែរ។ សូមវិភាគ productivity data។
...
{{
  ...
  "personality_type": "ប្រភេទបុគ្គលិកលក្ខណៈ",
  "key_findings": ["រកឃើញ 1", "រកឃើញ 2"],
  "strengths": ["ចំណុចខ្លាំង 1"],
  "improvements": ["ចំណុចត្រូវកែ 1"],
  "patterns": ["Pattern 1"],
  "recommendations": ["ការណែនាំ 1", "ការណែនាំ 2"]
}}

Score 0-100។ ភាសាខ្មែរធម្មជាតិ។ ឆ្លើយតែ JSON។
"""
...
async def start_command(update, context):
    welcome = (
        ...
        "_• សុក្រ ៨:០០ - Weekly PDF_\n"
        "_• អាទិត្យ ២០:០០ - AI Summary_"
    )
...
async def week_command(update, context):
    ...
    try:
        ...
        pdf_bytes = await loop.run_in_executor(None, generate_week_calendar_pdf, monday)
        end = monday + timedelta(days=6)

        pdf_bytes.name = f"week_{monday.strftime('%Y%m%d')}.pdf"
        caption = f"📅 *កាលវិភាគសប្តាហ៍នេះ*\n📆 {monday.strftime('%Y-%m-%d')} → {end.strftime('%Y-%m-%d')}"
...
async def month_command(update, context):
    ...
    try:
        ...
        loop = asyncio.get_event_loop()
        pdf_bytes = await loop.run_in_executor(None, generate_month_calendar_pdf, year, month)
...
async def sync_command(update, context):
    await update.message.reply_text("🔄 កំពុង sync...")
    try:
        loop = asyncio.get_event_loop()
...
async def summary_command(update, context):
    ...
    try:
        ...
        loop = asyncio.get_event_loop()
        data = await loop.run_in_executor(None, generate_weekly_summary_ai, week_start, week_end)
...
async def briefing_command(update, context):
...
async def insights_command(update, context):
    await update.message.reply_text("🧠 កំពុងវិភាគ insights...")
    try:
        loop = asyncio.get_event_loop()
        data = await loop.run_in_executor(None, generate_insights_ai)

        if not data:
            await update.message.reply_text("📭 ត្រូវការ events យ៉ាងតិច ៥")
            return
...
async def send_weekly_reminder(context):
    ...
    try:
        ...
        loop = asyncio.get_event_loop()
        pdf_bytes = await loop.run_in_executor(None, generate_week_calendar_pdf, next_monday)
        end = next_monday + timedelta(days=6)

        pdf_bytes.name = f"week_{next_monday.strftime('%Y%m%d')}.pdf"
        caption = f"🔔 *រំលឹកកាលវិភាគសប្តាហ៍ក្រោយ*\n\n📅 {next_monday.strftime('%Y-%m-%d')} → {end.strftime('%Y-%m-%d')}"
...
async def send_personal_reminders(context):
    if not CHAT_ID:
        return
    try:
        now = datetime.now(TZ)
        ...
        for e in events:
            ...
            try:
                ...
                logger.warning(f"Reminder check: {ex}")
    except Exception as e:
...
async def send_weekly_ai_summary(context):
...
@flask_app.route("/")
def home():
    return "Voice Tracker Bot v3.5 is running!"
...
def run_bot():
    ...
    job_queue.run_daily(send_weekly_reminder, time=reminder_time, days=(4,), name="weekly_reminder")
    logger.info("Weekly reminder: Friday 8:00 AM")

    job_queue.run_repeating(send_personal_reminders, interval=1800, first=60, name="personal_reminders")
    logger.info("Personal reminders: Every 30 min")

    job_queue.run_repeating(sync_calendar_job, interval=900, first=30, name="calendar_sync")
    logger.info("Google Calendar sync: Every 15 min")

    weekly_ai_time = dtime(hour=20, minute=0, tzinfo=TZ)
    job_queue.run_daily(send_weekly_ai_summary, time=weekly_ai_time, days=(6,), name="weekly_ai_summary")
    logger.info("Weekly AI summary: Sunday 8:00 PM")

    briefing_time = dtime(hour=7, minute=0, tzinfo=TZ)
    job_queue.run_daily(send_daily_briefing, time=briefing_time, name="daily_briefing")
    logger.info("Daily briefing: Every day 7:00 AM")

    logger.info("Bot v3.5 is running!")
